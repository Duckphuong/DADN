...
export function AIPredictionPanel({ historyItem }: AIPredictionPanelProps) {
    const [predictionData, setPredictionData] = useState<HistoryData | null>(
        null,
    );
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-[10px] text-gray-400">
                    ...
                    {predictionData.created_at
                        ? new Date(predictionData.created_at).toLocaleString()
                        : 'N/A'}
                </span>
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-200">
                    ...
                    <div className="relative h-3 bg-gray-100 rounded-full overflow-hidden">
                        <div
                            ...
                            style={{
                                width: `${Math.min(100, Math.max(0, wqi.score))}%`,
                            }}
                        ></div>
                    </div>
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-200">
                    <div className="flex items-center gap-2 mb-3">
                        <AlertTriangle
                            className={`w-5 h-5 ${risk.status === 'Low Risk' ? 'text-green-500' : 'text-yellow-500'}`}
                        />
                        <span className="text-gray-900 font-medium">
                            Contamination Risk
                        </span>
                    ...
                    <div className="mt-3 flex gap-2">
                        ...
                        <div
                            className={`flex-1 h-2 rounded-full ${risk.risk_level >= 0 ? 'bg-green-400' : 'bg-gray-200'}`}
                        ></div>
                        <div
                            className={`flex-1 h-2 rounded-full ${risk.risk_level >= 1 ? 'bg-yellow-400' : 'bg-gray-200'}`}
                        ></div>
                        <div
                            className={`flex-1 h-2 rounded-full ${risk.risk_level >= 2 ? 'bg-red-400' : 'bg-gray-200'}`}
                        ></div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-200">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            <span
                                className={`${forecast.trend === 'Stable' ? 'text-green-600' : 'text-yellow-600'} text-sm flex items-center gap-1`}
                            >
                                <TrendingUp
                                    className={`w-4 h-4 ${forecast.trend !== 'Stable' ? 'animate-pulse' : ''}`}
                                />
                                {forecast.trend}
                            </span>
                        </div>
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-indigo-600 font-semibold">
                                {Math.round(forecast.predicted_wqi_range[0])} -{' '}
                                {Math.round(forecast.predicted_wqi_range[1])}
                            </span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-gray-600 text-sm">
